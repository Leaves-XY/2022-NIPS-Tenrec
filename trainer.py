...
def mtlTrain(model, train_loader, val_loader, test_loader, args, train=True):
    ...
    path = os.path.join(
        args.save_path,
        '{}_{}_seed{}_best_model_{}.pth'.format(
            args.task_name, args.model_name, args.seed, args.mtl_task_num
        )
    )
    ...
    if args.mtl_task_num == 2:
        if train:
            ...
            for i in range(epoch):
                y_train_click_true = []
                y_train_click_predict = []   # 存概率
                y_train_like_true = []
                y_train_like_predict = []    # 存概率
                total_loss, count = 0, 0

                for idx, (x, y1, y2) in enumerate(train_loader):  # x:特征 | y1:点击标签 | y2:喜欢标签
                    x, y1, y2 = x.to(device), y1.to(device), y2.to(device)
                    predict = model(x)  # predict[0], predict[1] 是 logits

                    # 概率（用于 AUC & LogLoss）
                    click_prob = torch.sigmoid(predict[0]).squeeze().cpu().detach().numpy()
                    like_prob = torch.sigmoid(predict[1]).squeeze().cpu().detach().numpy()

                    y_train_click_true += list(y1.squeeze().cpu().numpy())
                    y_train_like_true += list(y2.squeeze().cpu().numpy())
                    y_train_click_predict += list(click_prob)
                    y_train_like_predict += list(like_prob)
                ...
                click_logloss = log_loss(y_train_click_true, y_train_click_predict)
                like_logloss = log_loss(y_train_like_true, y_train_like_predict)

                log_print(
                    "Epoch %d train loss %.3f | click_auc %.3f logloss %.3f | like_auc %.3f logloss %.3f" %
                    (i + 1,
                     total_loss / count,
                ...
                with torch.no_grad():
                    for idx, (x, y1, y2) in enumerate(val_loader):
                        ...
                        click_prob = torch.sigmoid(predict[0]).squeeze().cpu().numpy()
                        like_prob = torch.sigmoid(predict[1]).squeeze().cpu().numpy()

                        y_val_click_true += list(y1.squeeze().cpu().numpy())
                        y_val_like_true += list(y2.squeeze().cpu().numpy())
                        y_val_click_predict += list(click_prob)
                        y_val_like_predict += list(like_prob)
                ...
                click_logloss = log_loss(y_val_click_true, y_val_click_predict)
                like_logloss = log_loss(y_val_like_true, y_val_like_predict)

                log_print(
                    "Epoch %d val   loss %.3f | click_auc %.3f logloss %.3f | like_auc %.3f logloss %.3f" %
                    (i + 1,
                ...
                # early stopping 仍然用总 loss（loss_1 + loss_2）
                if i == 0:
                    ...
                    if total_eval_loss / count_eval < eval_loss:
                        ...
                        if patience < early_stop:
                            ...
                            log_print(
                                "val loss is not decrease in %d epoch and break training" % patience,
                                args
                            )
                            break
        ...
        with torch.no_grad():
            for idx, (x, y1, y2) in enumerate(test_loader):
                ...
                y_test_click_predict += list(click_prob)
                y_test_like_predict += list(like_prob)
        ...
        click_logloss = log_loss(y_test_click_true, y_test_click_predict)
        like_logloss = log_loss(y_test_like_true, y_test_like_predict)

        log_print(
            "Test      loss %.3f | click_auc %.3f logloss %.3f | like_auc %.3f logloss %.3f" %
    ...
    # ================= 单任务分支（mtl_task_num != 2） =================
    else:
        if train:
            model.train()
            for i in range(epoch):
                y_train_label_true = []
                y_train_label_predict = []   # 概率
                total_loss, count = 0, 0

                for idx, (x, y) in enumerate(train_loader):
                    x, y = x.to(device), y.to(device)
                    predict = model(x)
                    prob = torch.sigmoid(predict[0]).squeeze().cpu().detach().numpy()

                    y_train_label_true += list(y.squeeze().cpu().numpy())
                    y_train_label_predict += list(prob)
                ...
                logloss_metric = log_loss(y_train_label_true, y_train_label_predict)
                log_print(
                    "Epoch %d train loss %.3f | auc %.3f logloss %.3f" %
                    (i + 1, total_loss / count, auc, logloss_metric),
                    args
                )
                ...
                with torch.no_grad():
                    for idx, (x, y) in enumerate(val_loader):
                        x, y = x.to(device), y.to(device)
                        predict = model(x)
                        prob = torch.sigmoid(predict[0]).squeeze().cpu().numpy()

                        y_val_label_true += list(y.squeeze().cpu().numpy())
                        y_val_label_predict += list(prob)
                ...
                logloss_metric = log_loss(y_val_label_true, y_val_label_predict)
                log_print(
                    "Epoch %d val   loss %.3f | auc %.3f logloss %.3f" %
                    (i + 1, total_eval_loss / count_eval, auc, logloss_metric),
                    args
                )

                # early stopping 仍按 loss
                if i == 0:
                    eval_loss = total_eval_loss / count_eval
                else:
                    if total_eval_loss / count_eval < eval_loss:
        ...
        with torch.no_grad():
            for idx, (x, y) in enumerate(test_loader):
                ...
                y_test_label_predict += list(prob)
        ...
        logloss_metric = log_loss(y_test_label_true, y_test_label_predict)
        log_print(
            "Test      loss %.3f | auc %.3f logloss %.3f" %
            (total_test_loss / count_eval, auc, logloss_metric),
            args
        )
